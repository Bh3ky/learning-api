...
@app.get(
    "/",   # Decorator: statement added above the function to give special attributes to it.
...
@app.get(
    "/v0/players/",
    response_model=list[schemas.Player],  # response_model=list[schemas.Player] informs FastAPI that the data returned from this endpoint is a list of Pydantic Player objects defined in the schemas.py file.
    summary="Get all the SWC players that meet all the parameters you sent with your request",
    description="""Use this endpoint to get a list of SWC players. You can use the parameters to filter down the players in the list. Names are not unique. You can skip and limit to perform pagination of the API. Don't use the Player ID values to perform counts. Those are not guaranteed to be in order.""",
...
def read_players(
    ...
    first_name: str = Query(
        None, description="The first name of players to return"
    ),
    last_name: str = Query(
    ...
    players = crud.get_players(
        db,  # get_player() function performs the database query.
        skip=skip,
        limit=limit,
...
@app.get(
    ...
    description="If you have an SWC Player from another API call such as v0_get_players, you can call this API using the player ID",
...
@app.get(
    ...
    summary="Get all the weekly performances that meet all the parameters you can sent with your request",
...
@app.get(
    ...
    description="""Use this endpoint to get lists of SWC fantasy football leagues. You use the skip and limiy to perform pagination of the API. League name is not guaranteed to be unique. Don't use the League ID for counting or logic, because that is an internal ID and is not guaranteed to be sequential""",
...
def read_leagues(
    skip: int = Query(
        0, description="The number of items to skip at the beginning of the API call."
    ),
    limit: int = Query(
        100, description="The number of records to return after the skippped records."
    ),
    minimum_last_changed_date: date = Query(
        None,
        description="The minimum data of change that you want to return records. Exclude any records changed before this.",
...
@app.get(
    ...
    description="""Use this endpoint to count the number of leagues, teams, and players in the SWC fantasy football. Use in combination with skip and limit in v0_get_leagues, v0_get_teams, or v0_get_players. Use this endpoint to get counts instead of making calls to the other APIs""",
    response_description="A list of teams on the SWC fantasy football website.",
